...
从上面的路由配置可以看出，除了首页之外，其他页面都是使用 `import()` 导入的，这样我们的项目便实现了代码分割。如果用户访问我们的站点首页，那么只会加载布局组件 `MainLayout` 和首页 `HomePage` 两部分的代码，其他页面的代码不会立刻加载，而是等到用户访问对应页面的时候再去加载。
...
- `/* webpackPrefetch: true */`：当前模块如果引入了某个带有此注释的模块，那么 Webpack 注入的代码会放置一个 `` 标签在 `<head>` 里，浏览器会去尝试在空闲时间发送请求预获取这些资源（一定是在当前模块加载完毕后），并在短时间内缓存；
- ` /* webpackPreload: true */`：当前模块如果引入了某个带有此注释的模块，那么 Webpack 注入的代码会放置一个 `<link rel="preload" as="script" href="模块文件名.js">` 标签在 `<head>` 里，浏览器会去尝试尽快请求这些资源，优先级比 prefetch 更高；**需要注意的是，Webpack 会试图在父模块开始加载时便放置 `<link>` 标签（也就是时机比父模块的加载还要早），如果父模块一开始就是同步加载的，那么 preload 不会生效**。

一般来说，使用 prefetch 时负担会更小一点，preload 用不好可能会导致性能优化无效。
