...
本文的内容部分参考自阮一峰博客的 [一篇博文](http://www.ruanyifeng.com/blog/2016/04/cors.html) 和 [MDN](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Access_control_CORS)，关跨域访问的其他解决方案可以参考阮一峰的 [这篇博文](http://www.ruanyifeng.com/blog/2016/04/same-origin-policy.html)，有关 CORS 的兼容性，可以在 [CanIUse](https://caniuse.com/#feat=cors) 上看到。
...
- LocalStorage、IndexDB（新一代的本地数据存储）、WebSQL（更新一代的数据存储） 跨域无法互通；
- 使用 `@font-face` 跨域加载字体；
...
- canvas 的 `drawImage` API 可以跨域加载图片，但获取图片的数据或转为 base64 等操作会被阻止；
- 使用 `<form>` 向跨域的站点提交表单。
