...
函数式编程是一种特别的编程范式（paradigm），这种编程范式的主旨是把运算过程写成一系列嵌套函数的调用。
...
可以通过这篇 [阮一峰的函数式编程初探](https://www.ruanyifeng.com/blog/2012/04/functional_programming.html) 来简单了解函数式编程，这篇文章写于 2012 年。
...
因为函数中存在可修改的状态，这其实也是一种 “副作用”，会导致其中的每一步计算的数据来源就存在了不确定性。所以，函数式编程追求 “无状态” 这一要求。
...
这种方式也简单的实现了函数的组合，同时达成了函数式编程无状态的要求。
...
通过用例，可以得出结论：**柯里化使我们更容易调整函数的参数的数量、位置，并以此来实现函数之间的组合。**



# 函数式编程工具：`ramda`

上面提到的 `pipe()`、`curry()` 等函数，它们都是函数式开发中很基本的概念，实际开发中都是直接从现成的工具中引入并使用，并不需要我们自己来实现。

而这个函数式编程的工具，就是 [`ramda`](https://ramdajs.com/)，也可以点此访问 [中文文档](https://ramda.cn/)。注意是 **ramda** 不是 **rambda**，别拼错了。
...
上面提到的 `pipe()` 运算符，`ramda` 也实现了，和我们自己实现的稍有不同：
...
可以看出 `lodash` 的方法可能会修改原始参数，但 `ramda` 不会。



## `lodash/fp` 函数式 API

上面说了那么多，只是为了体现函数式编程和非函数式编程的区别。
...
- 对于 `ramda` 中不支持 `Promise` 的函数，`ramda-adjunct` 添加了支持 `Promise` 的函数，这些函数以 `P` 作为后缀，例如：
  `reduceP()` 提供了数组的异步折叠计算支持；
  `allP()` 可以等待数组中所有 `Promise` 类型已完成再返回结果。

- 对于对象数据：
  函数 `renameKeys()` 提供了对对象键名的重命名能力，而 `renameKeysWith()` 提供了对对象键名的变换的能力；
  函数 `omitBy()` 提供了依判断条件删除属性的能力；
  函数 `mergePaths()` 提供了精确控制合并路径的能力；
- 提供了 `isDate()`、`isArray()` 等一系列类型判断函数，这些函数还有 `iSNotDate()`、`isNotArray()` 的反向版本；

- 函数 `Y()` 提供了函数式编程的递归实现方式，它也是 lambda 推导中的 “Y 组合子”，请见本文最后一个章节，此处不再赘述。
...
我们可以使用函数式编程来实现这个递归函数，先来最简单的，把这个函数改写成表达式的形式：
...
这里，我们必须使用 `function` 语句，不然在开始下轮递归时，我们没有任何一个标识符来代指当前函数本身；

**不难看出，上面无论哪种方法，都必须声明一个变量或者方法来标识递归函数本身，否则递归函数便无法实现。**
而函数式编程旨在消除中间变量，但我们发现递归函数无法用纯函数组合来实现，一定需要使用中间变量。
...
我们已经知道了 `factorial2` 本身并不是真正执行的函数，真正执行的时候，使用的是 `factorial2(factorial2)`，所以表达式中的 `n * f(n - 1)` 肯定也是不对的，这里的 `f` 应该也跟着一起改成 `f(f)` 才对。
...
可以看到，我们成功用函数 `Y3` 封装了上面步骤 ③ 的操作。

继续封装步骤 ② 的操作：
步骤 ② 中，将 `a => b => a(b)` 改写为了 `a => b => a(a)(b)`，可以封装为：

```ts
const Y2 = a => b => a(x => b(b)(x))
```
...
```ts
const Y = (a => b => a(a)(b))(a => b => a(x => b(b)(x)))
```
