...
建议自己时间一下，把原来的代码替换成简化版的，体验一下这个 API 的强大。
...
第一种方式，是使用 `useLayoutEffect()` 取代 `useEffect()`，因为它会在 DOM 变动后同步触发，此时浏览器预备的第一次 “布局” 还没有开始，设置好偏移后，然后再 `requestAnimationFrame()` 的回调中修改属性，也就是图中的绿色区域，引发一次 “重排”，这样过渡就能正常生效了。
