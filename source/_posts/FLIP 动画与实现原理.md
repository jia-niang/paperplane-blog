...
实际上，要想实现上述动画效果，即使使用了 Vue，也必须配合 Vue 的内置组件 `<transition-group>` 来实现，这个组件专为一组 DOM 进行顺序变化的场合而设计，例如列表插入、列表移除、调整顺序等需求，它会在多个元素变动时调整元素的 CSS 属性，以此实现这种 FLIP 动画的效果。
...
原因很简单，因为我们在同步代码中设置样式，`transform` 最终被设置为空，自然没有动画产生。
...
`requestAnimationFrame()` 接受一个回调方法作为参数，这个回调将在浏览器下一次 “绘制” 之前执行，它的执行时机正符合我们的需求；所以，上面的步骤 ②③④，只需要在 `requestAnimationFrame()` 回调中运行，就能够既获取元素移动的终点坐标，又避免了瞬移。
...
此外，这里会使元素被设置到终点位置后，再通过 `transform` 偏移回到起点，这个过程中如果你担心用户能感知到，那么可以通过设置 `opacity` 等方式，避免元素被用户看到。
...
- 监听到 DOM 变动后，会产生一个 [`MutationRecord`](https://developer.mozilla.org/zh-CN/docs/Web/API/MutationRecord) 记录，对于子元素的增删变化，它的 `type` 属性值为 `'childList'`，此时：
  ...
  如果某个元素被移动了位置，但是没有被移除容器节点，那么会同时产生一条移除记录和一条增加记录，且它们的上述属性指向同一个元素；
