...
然后这一步是可选的，如果你使用域名来连接，建议做这一步；如果用 IP 直连，这一步可省略。
修改 hosts，把本机的域名指向本地。
执行指令：
...
这里需要注意的是，如果你通过 SSH 连接到服务器，那么此时这里的 “Host” 地址是相对于服务器而言的了。
**如果数据库就在这台服务器上，你可以把 “Host” 直接配置成 `localhost`；也可以像我这样填写本机的域名，那么上一步配置 hosts 便是必须的步骤，不然服务器走外网 DNS 到自身的地址，却发现 `5432` 端口被防火墙挡了。**
