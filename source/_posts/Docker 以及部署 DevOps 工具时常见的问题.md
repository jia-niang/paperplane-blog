...
遇到此问题，可以尝试给服务的 Docker Compose 配置添加这一项：
...
通常而言，Docker 会创建网关并为每个容器管理流量，对于端口映射而言，Docker 默认也会使用 `0.0.0.0` 绑定到宿主机，更常见的情况是使用 Nginx，流量被 Nginx 管理后也是来自 `127.0.0.1` 的。

所以，绝大多数情况，应用程序监听绑定 `127.0.0.1` 是可以直接使用的，不会出问题。
但如果将服务运行于 Docker 外直接对外暴露，或运行环境发生了变化，或涉及到 Docker 内外的服务之间的通讯，这种场合遇到了完全监听不到流量的情况，此时便可以参考上述方法来处理。
